...
export default function App() {
  ...
  const [classes, setClasses] = useState(() => {
    const saved = localStorage.getItem('compass-lallravi-v5');
  ...
  useEffect(() => { localStorage.setItem('compass-lallravi-v5', JSON.stringify(classes)); }, [classes]);
      ...
      <header className="h-24 bg-white border-b-4 border-slate-200 px-8 flex justify-between items-center z-50 shrink-0">
        <div className="flex flex-col">
          ...
          <div className="flex gap-3 mt-2">
            <button onClick={() => setActiveTab('seating')} className={`text-[10px] font-black px-4 py-1.5 rounded-full uppercase transition-all ${activeTab === 'seating' ? 'bg-indigo-600 text-white' : 'bg-slate-200'}`}>Seating & Groups</button>
            <button onClick={() => setActiveTab('noise')} className={`text-[10px] font-black px-4 py-1.5 rounded-full uppercase transition-all ${activeTab === 'noise' ? 'bg-indigo-600 text-white' : 'bg-slate-200'}`}>Noise Alarm</button>
          </div>
        </div>
        {activeTab === 'seating' && (
          <div className="flex gap-3">
            <button onClick={() => toPng(floorRef.current, { width: 5000, height: 4000 }).then(u => { const p=new jsPDF('l','px',[5000,4000]); p.addImage(u,'PNG',0,0,5000,4000); p.save(`${currentClassName}.pdf`); })} className="bg-emerald-600 text-white px-5 py-2.5 rounded-xl font-bold shadow-md">Save PDF</button>
            <button onClick={() => { if(currentData.students.length){ setIsRandomizing(true); let c=0; const i=setInterval(()=>{setPickedStudent(currentData.students[Math.floor(Math.random()*currentData.students.length)]);c++;if(c>20)clearInterval(i)},80);}}} className="bg-indigo-600 text-white px-5 py-2.5 rounded-xl font-bold flex items-center gap-2 shadow-md"><Target size={20}/> Randomizer</button>
          ...
          <>
            <aside className={`${isSidebarOpen ? 'w-80' : 'w-0'} bg-white border-r-4 border-slate-100 transition-all duration-300 overflow-y-auto z-40 shadow-xl`}>
              <div className="p-6 w-80 space-y-6">
                ...
                {/* GROUP MAKER TOOL - INTEGRATED */}
                <div className="p-4 bg-indigo-50 rounded-3xl border-2 border-indigo-100 space-y-4">
                    <label className="text-[10px] font-black text-indigo-400 uppercase tracking-widest flex items-center gap-2"><Users size={14}/> Group Maker Tool</label>
                    <div className="flex items-center justify-between">
                        <span className="text-xs font-bold text-indigo-900">Students / Group:</span>
                        <input type="number" min="1" value={groupSize} onChange={(e) => setGroupSize(parseInt(e.target.value))} className="w-12 bg-transparent text-xl font-black text-indigo-600 text-right outline-none" />
                    </div>
                    <button onClick={handleShuffle} className="w-full py-3 bg-indigo-600 text-white rounded-xl font-black text-xs uppercase flex items-center justify-center gap-2 hover:bg-indigo-700 transition-all shadow-md">
                        <Shuffle size={16}/> Create Random Groups
                    </button>
                    {generatedGroups.length > 0 && (
                        <button onClick={() => setGeneratedGroups([])} className="w-full text-[10px] font-black text-indigo-400 uppercase hover:text-red-500">Clear Groups</button>
                    )}
                </div>

                <div className="space-y-1">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Add Roster</label>
                    <textarea value={bulkNames} onChange={(e) => setBulkNames(e.target.value)} placeholder="Paste names..." className="w-full h-32 p-4 bg-slate-50 border-2 rounded-2xl text-xs font-medium outline-none focus:border-indigo-500" />
                    <button onClick={() => {
                      ...
                      setBulkNames("");
                    }} className="w-full py-4 bg-slate-900 text-white rounded-2xl font-black uppercase text-xs tracking-widest mt-2">Add Students to Floor</button>
                </div>

                <button onClick={() => { const n=prompt("CLASS NAME:")?.toUpperCase(); if(n) {setClasses(p=>({...p,[n]:{students:[],teacherDesk:{id:Date.now(),name:'TEACHER',x:2300,y:300,rotation:0}}})); setCurrentClassName(n);}}} className="w-full py-3 border-2 border-dashed border-slate-300 text-slate-400 rounded-2xl font-black text-[10px] uppercase hover:bg-slate-50">Create New Period</button>
              </div>
            </aside>
              ...
              <TransformWrapper initialScale={0.3} minScale={0.05} centerOnInit={true} limitToBounds={false} onZoom={(ref) => setCurrentScale(ref.state.scale)}>
                ...
                <TransformComponent wrapperStyle={{ width: "100%", height: "100%", cursor: "move" }}>
                  <div ref={floorRef} className="relative bg-white shadow-inner" style={{ width: '5000px', height: '4000px' }}>
                    ...
                    {/* DYNAMIC GROUP OVERLAY (Rendered on Canvas for PDF) */}
                    {generatedGroups.length > 0 && (
                        <div className="absolute left-[100px] top-[600px] grid grid-cols-2 gap-20 p-20 bg-slate-50/80 rounded-[5rem] border-[15px] border-indigo-600/20 backdrop-blur-sm z-10 shadow-2xl">
                             {generatedGroups.map((group, idx) => (
                                <div key={idx} className="bg-white p-12 rounded-[3rem] shadow-xl border-t-[15px] border-indigo-600 w-[800px]">
                                    <h3 className="text-4xl font-black text-indigo-600 mb-8 uppercase italic">Group {idx + 1}</h3>
                                    <div className="space-y-4">
                                    {group.map(s => <div key={s.id} className="text-5xl font-black text-slate-800 uppercase border-l-[12px] border-slate-100 pl-8 leading-tight">{s.name}</div>)}
                                    </div>
                                </div>
                            ))}
                        </div>
        ...
        {activeTab === 'noise' && (
          <div className={`flex-1 flex flex-col items-center justify-center transition-colors duration-500 ${volume > threshold ? 'bg-red-600 animate-pulse' : 'bg-slate-50'}`}>
            ...
            <button onClick={isMonitoring ? () => { setIsMonitoring(false); cancelAnimationFrame(animationRef.current); audioContext.current.close(); setVolume(0); } : async () => {
                ...
                update();
            }} className="mt-20 px-16 py-8 bg-indigo-600 text-white rounded-[3rem] font-black text-4xl shadow-2xl">
